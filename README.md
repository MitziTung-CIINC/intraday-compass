...
- 在 B 点或 S 点确认成熟时使用网页弹窗、不同提示音和系统通知提醒（B 点连续三声上扬音，S 点连续五声下行音）；
...
在“当前判断”卡片底部点击 **开启B/S弹窗与声音**。系统只在模型进入 B 点确认或 S 点确认状态时提醒，观察信号不会触发弹窗。

- **网页弹窗**：无需浏览器权限，但页面需要保持打开。
- **提示音**：不需要麦克风权限；首次点击开启按钮会解锁浏览器的声音播放能力。若浏览器或系统将网站静音，需要手动取消静音。
- **系统通知**：网站必须通过 HTTPS 打开，并在浏览器询问时选择“允许通知”。启用成功后会立即发送一条测试通知；B/S 成熟通知会进入系统通知中心，并在后台标签页中保持显示。若拒绝通知，网页弹窗和提示音仍可继续使用。
- **不需要的权限**：摄像头、麦克风、位置、通讯录、券商账户和资金账户权限均不需要。

提醒只表示模型条件已经成熟，不能替代对行情源时间、硬门槛、可卖数量、交易费用和预计差价的人工复核，也不会自动下单。
